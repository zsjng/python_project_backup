...
class ImageRecognition(QThread):
    ...
    def run(self):
        self.insert_text_signal.emit('开始文字识别, 如果视频文件过大，请等待')
        while True:
            if not self.task_queue.empty():
                ...
                self.insert_text_signal.emit('结束文字识别')
                self.start_btn_signal.emit(False)
                self.insert_content_signal.emit('\n\n\n\n')
                break
...
class MainWindow(QMainWindow):
    ...
    def init_tabs(self):
        ...
        tabs.addTab(self.tab1, "文字识别")
        tabs.addTab(self.tab2, "高级配置")
    ...
    def init_tab2_ui(self):
        ...
        video_setting_box = QGroupBox('文字识别参数', self)
